...
# This script takes the DICOM heart MRI images and converts them to PNG Grayscale images.
# The script applies several preprocessing steps on the images.
# In addition, the script can combine three of the 50 images of a scan in one RGB image.
...
def resample_spacing(image):
    ...
    new_spacing = (1, 1)
    new_size = [int(original_size[0]*original_spacing[0]),
                int(original_size[1]*original_spacing[1])]
...
def crop_to_square(image):
    ...
    start_index = tuple(map(
        int, [middle_index[0] - half_square_size, middle_index[1] - half_square_size]))
...
def main():
    ...
    parser.add_argument('--input_dir',
                        type=str,
                        default='/dhc/projects/ukbiobank/original/imaging/heart_mri/lax/archive',
    ...
    parser.add_argument('--output_dir',
                        type=str,
                        default='./out/10_RGB_0-16-39',
                        help='Path to output directory. By default it is ./out/10_RGB_0-16-39, because the images with the indices 0,16,39 of the first 10 patients are used to export 10 RGB images which combines the three images for each patient.')
    parser.add_argument('--create_subdirs',
                        type=bool,
                        default=False,
                        help='Create a subdirectory for every patient.')
    parser.add_argument('--export_format',
                        type=str,
                        default='png',
                        choices=['png', 'dcm'],
                        help='Export format for image files.')
    parser.add_argument('--create_zip',
                        type=bool,
                        default=False,
                        help='Create a zip file of the output directory.')
    parser.add_argument('--number_of_patients',
                        type=int,
                        default=10,
                        help='Number of patients to get image files from. By default images of 10 patients are exported. Set to -1 to export images of all patients.')
    parser.add_argument('--indices',
                        type=str,
                        default="0,16,39",
                        help='Indices of images to export e.g. "0,16,39". By default all images are exported.')
    parser.add_argument('--export_RGB_from_indices',
                        type=bool,
                        default=True,
                        help='Requires that exactly three indices are provided. Export one RGB image that combines the three greyscale images. The create_subdirs argument will be ignored.')
                ...
                try:
                    ...
                    for series_id in series_ids:
                        ...
                        series_file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(
                            patient_temp_dir, series_id)
                        series_reader.SetFileNames(series_file_names)
                        series_reader.MetaDataDictionaryArrayUpdateOn()
                        images = series_reader.Execute()
                        series_description_key = '0008|103e'
                        series_description = series_reader.GetMetaData(
                            0, series_description_key)
                        ...
                        if series_description == 'CINE_segmented_LAX_4Ch':
                            if args.export_RGB_from_indices:
                                ...
                                image_2_resampled = sitk.Resample(
                                    image_2, image_1)
                                image_3_resampled = sitk.Resample(
                                    image_3, image_1)
                                image = sitk.Compose(
                                    [image_1, image_2_resampled, image_3_resampled])
                                image = resample_spacing(image)
                                ...
                                for i in range(images.GetDepth()):
                                    ...
                                    if args.create_subdirs:
                                        ...
                                        os.makedirs(
                                            patient_output_dir, exist_ok=True)
                    ...
                    error_counter += 1
                    print(
                        f'An error occured for patient: {patient_id}. Number of errors occured: {error_counter}.')
                    print(e)
