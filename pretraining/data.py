...
class MriData(Dataset):
    ...
    def _load_item(self, idx):
        ...
        img = Image.open(p)
        ## Test
        ## for MRI -> Quickfix: convert + resize
        img = img.convert(('RGB'))
    ...
    def __getitem__(self, idx):
        ...
        if self.tfms:
            ...
            # output with shape [1, 448, 535] doesn't match the broadcast shape [3, 448, 535] -> fix in load data
            img = self.tfms(img)
        return img, label
...
class ClassificationMriData(MriData):
    def __init__(
        ...
        ## TODO: Here you need to switch to png if we read in pngs. 
