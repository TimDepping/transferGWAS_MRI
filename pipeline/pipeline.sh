...
feature_condensation=$(sbatch --parsable pipeline/feature_condensation.slurm $INPUT_INDIV_FILE $EXCLUDED_INDIV_FILE $OUTPUT_DIR $IMAGE_DIR $N_PCS $MODEL $PRETRAINING $LAYER)

# Run LMM
EMBEDDINGS_FILE=${OUTPUT_DIR}/${MODEL}_${MODEL_NAME}_$LAYER.txt
lmm=$(sbatch --parsable --dependency=afterok:$feature_condensation pipeline/lmm.slurm $OUTPUT_DIR $EMBEDDINGS_FILE $N_PCS)

# Plot and change file permissions of output dir
plot=$(sbatch --dependency=afterok:$lmm pipeline/plot.slurm $OUTPUT_DIR)
