...
def main():
    ...
    parser.add_argument('--config', type=str,
                        help='Parse arguments from config file')
    parser.add_argument(
        '--bed', type=str, help='path to microarray bed file (multiple files e.g. via `foo_{1:22}.bed`)')
    parser.add_argument(
        '--bim', type=str, help='path to microarray bim file (multiple files e.g. via `foo_{1:22}.bim`)')
    ...
    parser.add_argument(
        '--qcov_cols',
        type=str,
        default=['age', 'genet_PC_{1:41}'],
        nargs='+',
        help='quantitative covariate columns in covariate file',
    )
    parser.add_argument(
        '--emb',
        type=str,
    ...
    parser.add_argument('--out_fn', type=str, default='PC_%d.txt',
                        help='output filename; should include a `%d` for the PC')
    parser.add_argument(
        '--bolt', type=str, help='path to BOLT-LMM directory. If none specified, will try to download bolt-lmm to `bolt/`')
    parser.add_argument('--threads', default=130, type=int,
                        help='how many threads to use for BOLT-LMM')
    parser.add_argument('--max_missing_snp', default=0.1,
                        type=float, help='maximum missingness per SNP')
    parser.add_argument('--max_missing_indiv', default=0.1,
                        type=float, help='maximum missingness per individual')
    parser.add_argument('--ref_map', type=str,
                        help='Alternative genome reference map. If none specified, will use GRCh37/hg19 map from BOLT-LMM')
    parser.add_argument('--ldscores', type=str,
                        help='LDScores for test calibration. If none specified, will use European ancestry scores from BOLT-LMM')

    parser.add_argument('--run_imputed', action='store_true',
                        help='Set flag if you want to run analysis on imputed data')
    parser.add_argument('--out_fn_imp', default='PC_%d.imp.txt', type=str,
                        help='output filename for imputed; should include a `%d` for the PC')
    parser.add_argument(
        '--bgen', type=str, help='path to imputed .bgen files. Multiple files via `foo_{1:22}.bgen`')
    parser.add_argument(
        '--sample', type=str, help='path to imputed .sample files. Multiple files via `foo_{1:22}.sample`')
    parser.add_argument('--imp_maf', default=0.001, type=float,
                        help='Minor allele frequency in imputed data.')
    parser.add_argument('--imp_info', default=0.001, type=float,
                        help='Imputation information quality score in imputed data.')
    parser.add_argument(
        '--remove',
        type=str,
        default=[],
    ...
    if args.config is not None:
        ...
        if not args.ref_map:
            configs['geneticMapFile'] = join(
                bolt, 'tables', 'genetic_map_hg19_withX.txt.gz')
        if not args.ldscores:
            configs['LDscoresFile'] = join(
                bolt, 'tables', 'LDSCORE.1000G_EUR.tab.gz')
    ...
    if in_transform in ['marginal', 'adjusted']:
        ...
        fn = join(tmp_dir, '.'.join(configs['phenoFile'].split(
            '/')[-1].split('.')[:-1]) + f'_INT_{in_transform}_{uuid.uuid4()}.txt')
    ...
    for pc in pcs:
        ...
        run_single_bolt(bolt, flags, covars, qcovars,
                        remove, log_file, **configs)
...
def check_bolt(pth):
    ...
    if not os.path.isdir(pth):
        ...
        request = requests.get(
            'https://storage.googleapis.com/broad-alkesgroup-public/BOLT-LMM/downloads/old/BOLT-LMM_v2.3.4.tar.gz')
