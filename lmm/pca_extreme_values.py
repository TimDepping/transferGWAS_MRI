...
# This script copies the images of indidivuals with extreme PC values and creates a csv file that contains their IIDs and the PC values.

def copy_image(iid, source_dir, output_dir):
    files_with_iid = [filename for filename in os.listdir(
        source_dir) if filename.startswith(str(iid))]
...
def main():
    ...
    parser.add_argument(
        'input', type=str, help='Textfile which includes principal components for each individual e.g. resnet50_imagenet_L4.txt')
    parser.add_argument('--output_dir', type=str, default="./out",
                        help='Directory to save the image files')
    parser.add_argument('--n_extreme', type=int, default='10',
                        help='Number of extreme values to extract')
    ...
    ids = df["IID"]
    print(df[ids.isin(ids[ids.duplicated()])].sort_values("IID")["IID"])
