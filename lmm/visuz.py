...
class general():
    ...
    def get_figure(show, r, figtype, fig_name):
        if show and fig_name is not None:
...
class marker():
    def geneplot_mhat(df, markeridcol, chr, pv, gwasp, markernames, gfont, grot, ax):
        if markeridcol is not None:
            if markernames is not None and markernames is True:
                ...
                #for i in df[markeridcol].unique():
                    plt.text(df.loc[df[markeridcol] == i, 'ind'].iloc[0], df.loc[df[markeridcol] == i, 'tpval'].iloc[0],
                                 str(i), fontsize=gfont, rotation=grot)
    ...
    def mhat(df="dataframe", chr=None, pv=None, bp=None, color=None, dim=(6,4), r=300, ar=90, gwas_sign_line=False,
             gwasp=5E-08, dotsize=8, markeridcol=None, markernames=None, gfont=8, grot=0, valpha=1, show=False, figtype='png',
             axxlabel=None, axylabel=None, axlabelfontsize=9, axlabelfontname="Arial", axtickfontsize=9,
             axtickfontname="Arial", ylm=None, plot_extra=None, save_str=None, lower_lim=None, main_marker='.'):
        ...
        if not lower_lim is None:
            df['too_small'] = df[pv] < lower_lim
            too_small_pvs = df.loc[df.too_small, pv].values
            df.loc[df.too_small, pv] = lower_lim
        ...
        if not bp is None:
            df['ind'] = 0
            for ch in sorted(df[chr].unique()):
                last_ind = df.ind.max()
                C = df[chr]==ch
                df.loc[C, 'ind'] = df.loc[C, bp]+last_ind
        else:
            df['ind'] = range(len(df))
        ...
        for label, df1 in df.groupby(chr):
            x = 'ind'
            df1.plot(kind='scatter', x=x, y='tpval', color=color_list[i], s=dotsize, alpha=valpha, ax=ax, marker=main_marker, rasterized=True)
            if len(df1) > 1:
                df1_max_ind = df1['ind'].iloc[-2]
                df1_min_ind = df1['ind'].iloc[1]
            else:
        ...
        if not lower_lim is None:
            ...
            '''
            dff = df[df.too_small]
            for j, (i, row) in enumerate(dff.iterrows()):
                print(row[pv], row.ind, row.tpval)
                s = f'\n\t-----------{too_small_pvs[j]:.1E}'
                plt.annotate(s, (row.ind, row.tpval))
            '''
        if plot_extra:
            i = 0
            #cmap_colors = 5*plt.get_cmap('tab10').colors
            #cmap_colors = 5*sns.color_palette('colorblind')
            #cmap_colors = 5*plt.get_cmap('Set1').colors
            #cmap_colors = ['r', 'g', 'b', 'y', 'k']
            ...
            mpl_markers = 5*['o', 'X', '*', 'v', '^', '<', '>', 's', 'p', 'P', 'D', '1', '2', '3', '4']
            for label, df1 in df.groupby(plot_extra):
                df1 = df1.loc[df1[pv]<gwasp]
                #plt.scatter(df1.ind.values, df1.tpval.values, s=5*dotsize, marker=mpl_markers[i], alpha=valpha, label='PC %d' % (label+1))
                plt.scatter(df1.ind.values, df1.tpval.values, color=cmap_colors[i], s=8*dotsize, marker=mpl_markers[i], alpha=valpha, label='Dim %d' % (label+1), rasterized=True)
                #df1.plot(kind='scatter', x='ind', y='tpval', color=[cmap_colors[i]], s=5*dotsize, marker=mpl_markers[i], alpha=valpha, ax=ax, label='PC %d' % (label+1))
                i += 1
            ax.legend(loc='upper center', ncol=5, fancybox=True, shadow=False, bbox_to_anchor=(0.5, 1.05), fontsize=axtickfontsize) 
            if len(np.unique(df[plot_extra])) > 12:
                ax.legend(loc='upper center', ncol=15, fancybox=True, shadow=True, bbox_to_anchor=(0.5, 1.1)) 
        ...
        if markernames is not None:
            marker.geneplot_mhat(df, markeridcol, chr, pv, gwasp, markernames, gfont, grot, ax=ax)
        ax.margins(x=0)
        ax.margins(y=0)
        ...
        if ylm:
            ...
            ylm = np.arange(0, max(df['tpval']+1), 5)
        ax.set_yticks(ylm.astype(int))
        ax.set_xticklabels(xlabels, rotation=ar, fontsize=axtickfontsize)
        ax.set_yticklabels(ylm.astype(int), fontsize=axtickfontsize, fontname=axtickfontname, rotation=ar)
        if axxlabel:
            _x = axxlabel
        if axylabel:
            _y = axylabel
        ...
        sns.despine(ax=ax)
        general.get_figure(show, r, figtype, save_str)
